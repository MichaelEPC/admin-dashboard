const TaskAsignItem = async ({
  ...
  return (
    <div className="mt-1 flex h-auto w-full flex-col rounded-lg border-2 border-ligh-gray bg-white pb-2 shadow-sm">
      <div className="mt-2 flex h-auto w-full flex-col items-center justify-between border-b-2 border-ligh-gray px-4 md:flex-row">
        <p className="text-lg font-semibold">{`Name: ${name}`}</p>
        <p className="text-lg font-semibold text-principal-color">{job}</p>
        <p className="text-lg font-semibold">{`Task asign: ${task}`}</p>
      </div>
    </div>
  );
};
