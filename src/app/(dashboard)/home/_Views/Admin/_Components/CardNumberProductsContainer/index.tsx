"use server";

import { CardInformative } from "app/app/_components/CardsInformationGraphic";
import { getTodayDate } from "../utils";
import { getProductsNumber } from "./GetProductsNumber";

export const CardProductsContainer = async () => {
  const date = await getTodayDate();
  const number = await getProductsNumber();

  let svgPaths = [
    `M21.93 7.66c-.02-.05-.04-.11-.07-.16a.6.6 0 0 0-.06-.08c-.03-.04-.06-.09-.1-.12-.03-.03-.06-.04-.09-.07-.04-.03-.07-.06-.11-.09h-.01l-9-5.01a.99.99 0 0 0-.97 0L2.59 7.1s-.06.02-.09.04a.99.99 0 0 0-.49.86v8c0 .36.2.7.51.87l9 5 .15.06c.03.01.06.03.09.03a1.1 1.1 0 0 0 .5 0c.03 0 .06-.02.09-.03.05-.02.1-.03.15-.06l9-5c.32-.18.51-.51.51-.87V8c0-.09-.01-.18-.04-.26 0-.03-.02-.05-.03-.08ZM12 4.15l6.94 3.86-2.24 1.25-6.84-3.91L12 4.16ZM7.82 6.48l6.84 3.91-2.65 1.47L5.09 8l2.74-1.52ZM4 9.7l7 3.89v5.71l-7-3.89zm16 5.71-7 3.89v-5.71l3-1.67V15l2-1v-3.19l2-1.11z`,
  ];

  return (
    <div>
      {/* @ts-ignore */}
      <CardInformative
        title={"Amount of products:"}
        principalText={`${number}`}
        svgPath={svgPaths}
        date={`${date.month}-${date.day}-${date.years}`}
      />
    </div>
  );
};
