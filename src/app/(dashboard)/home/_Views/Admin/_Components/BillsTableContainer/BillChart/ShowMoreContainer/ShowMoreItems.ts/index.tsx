...
const ShowMoreItems = ({ id, name, amount, category, date }: ControlProps) => {
  return (
    <article className="mb-2 h-16 w-full bg-white px-2">
      <div className="flex h-auto w-full justify-between">
        <p className="font-semibold text-text-color">{name}</p>
        <p className="text-base font-bold text-black">{`Amount: $${amount}`}</p>
      </div>
