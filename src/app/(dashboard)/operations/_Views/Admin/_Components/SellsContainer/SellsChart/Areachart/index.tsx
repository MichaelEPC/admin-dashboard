...
export const ComboChartOperation: React.FC<OperationItemsProps> = ({
  list,
}: OperationItemsProps) => {
  ...
  const solarPanelsValue =
    payload?.SolarPanels ?? dataa[dataa.length - 1].SolarPanels;
  const momChangeValue =
    payload?.MoMChange ?? dataa[dataa.length - 1].MoMChange;
      ...
      <div>
        ...
        <ComboChart
          ...
          barSeries={{
            ...
            valueFormatter: (v: any) => currencyFormatter(v),
          }}
          lineSeries={{
            colors: ["gray"],
            categories: ["MoMChange"],
            valueFormatter: (v: any) => percentageFormatter(v),
          }}
